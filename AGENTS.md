...
- If a gcloud-wrapper deploy times out while polling, do not immediately start another deploy. Query the exact App Hosting build/rollout through Firebase Console or the App Hosting API; if the build is `READY` and the rollout is `SUCCEEDED`, report the wrapper timeout separately and continue with route/session smoke from that deployed commit.
...
- Result pages should poll until `completed` or `failed` while open, but must not poll forever when evaluation start already failed (`startFailed=1`). Customer-facing report text must hide raw/debug output and mechanical turn IDs.
