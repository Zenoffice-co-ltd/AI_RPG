...
- For v50-family App Hosting deploys through the generic gcloud wrapper, pass the variant (`--variant v50-7` / `--variant v50-8`) so the post-check validates `/api/grok-first-v50*/session`; batch runtime/router/guard fixes and skip deploys for docs-only, runner-only, or unit-test-only edits.
