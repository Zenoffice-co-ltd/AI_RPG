...
v13 で `soft_timeout_config: null` を明示送信する変更を試みたが、ElevenLabs API は `Invalid conversation config: Input should be a valid dictionary or instance of SoftTimeoutConfig` と reject。**ElevenLabs API は `null` を受け付けない** ため、payload からの省略 (omit) しか選択肢が無い。結論として omit-when-undefined を維持。

### 結果として確立したルール (v13)

1. **クリアしたい vendor フィールドは `null` を試す前に、ElevenLabs API がそれを accept するか確認する**。Adecco の `soft_timeout_config` は `null` を accept しなかったため、dashboard 側の手動クリアが必須。
2. **dashboard で設定された vendor 側 filler は publish では消えない** ことを runbook に明記する。operator は publish 後に dashboard を点検し、不要な filler / soft timeout を手動でクリアする必要がある。
3. **future workaround 候補** (要 dashboard 検証):
   - `{ timeout_seconds: 86400, message: "" }` を送って機能的に無効化する
   - ElevenLabs admin API で field を PUT して explicit clearing semantics を使う
   - これらは v14+ で安全性を確認してから採用
...
5. **教訓 (v13 後付け)**: 「null を送って clear」の試みは ElevenLabs API では機能しない。同様の問題が `silenceEndCallTimeoutSeconds` `turnEagerness` `spellingPatience` `speculativeTurn` `retranscribeOnTurnTimeout` `mode` などにも潜む可能性があるが、null clearing の有効性は **case-by-case で verify** する必要がある。
...
## Manual Orb v14 lesson: 禁止フレーズの literal を prompt に書かない (2026-04-27)

**実環境 LLM の重要前提**: Adecco orb の本番 LLM は ElevenLabs ダッシュボード側で **GLM-4.5-air** に設定されている (publish snapshot には乗らない)。`.env.local.example` の `DEFAULT_ELEVEN_MODEL=gpt-5-mini` は publish-time の env 候補値で、ライブの実モデルではない。

### GLM-4.5-air の特性 (経験則)

- 軽量で **低レイテンシ**
- gpt-5-mini と比べて **指示追従の robustness が低い**:
  - Pattern 1 conflict (allow vs ban) で **allow 側に倒れやすい** (v13 で対応済)
  - **literal な禁止例文を「使ってよい例」として出力にコピーしやすい** ← v14 で発覚
  - 複雑な intent matching で off-by-one を起こしやすい (v13 semantic label で部分対応)

### v14 で発覚した literal-copy 問題

[compileStaffingReferenceScenario.ts:386](packages/scenario-engine/src/compileStaffingReferenceScenario.ts:386) の旧文言:
```
「Stage direction / メタ動作描写を発話してはいけない: 『（何も返さず、ユーザーの次の発話を待ちます）』『（沈黙）』『（応答なし）』のような括弧付きの動作描写・ト書きを出力に含めてはいけません。」
```

GLM-4.5-air はこの **禁止例文の literal `（何も返さず、ユーザーの次の発話を待ちます）` をコピー** して、沈黙時に音声出力した。同じ問題は `[slow]` `[pause]` `<break/>` 等の SSML literal にも潜在。

### v14 で確立したルール

1. **「禁止フレーズの literal 例」を prompt に書かない**。代わりに **カテゴリ + 抽象説明** だけ書く:
   ```
   ❌ "『（何も返さず、ユーザーの次の発話を待ちます）』『（沈黙）』のような括弧付きの動作描写を出力に含めない"
   ✅ "自分の内部動作 (沈黙する・無応答にする・待機する 等) を括弧書きで実況する文や、ト書き形式で書いた説明を出力に含めない"
   ```
2. literal な禁止例は **`negativeExamples` (test fixture only)** にだけ置く。`renderDisclosureLedgerForPrompt` は `negativeExamples` を live prompt に流さないので安全。
3. Pre-commit grep:
   ```bash
   # rendered prompt に literal な括弧書きや SSML タグが残っていないか
   grep -nE '『（|『\[slow\]|『\[pause\]|『<break|『<emphasis' \
     packages/scenario-engine/src/compileStaffingReferenceScenario.ts \
     packages/scenario-engine/src/disclosureLedger/staffingAdeccoLedger.ts
   ```
   ヒット 0 を維持。
4. `# Response Opening Format` の literal 禁止例 (`「承知しました。」`「**承知しました。少し整理しますね。** 指揮命令者の課長は...」 等) は **v9-v11 で効果実証済み** で literal-copy が観測されていないため、**v14 では touch しない** (動いている部分は触らない)。GLM-4.5-air が将来これらも copy し始めたら v15+ で対応。
5. **教訓**: 「禁止例を書く」ことは LLM 種類によっては逆効果。robust な gpt-4 系では効くが GLM-4.5-air では失敗する。**LLM 切替時に prompt strategy も再評価が必要**。

### 長期検討 (v15+)

- ElevenLabs で **GLM-4.5-air → gpt-4o-mini / gpt-5-mini に切り替え** 比較 (有償変動はあるが robustness 改善見込み)
- 21 個の質問意図を **decision tree / 早期リターン構造** に再編して GLM-4.5-air の弱点を補う
- `closing_summary.intentDescription` (~50 行) を更に圧縮 (recency bias 対策)
