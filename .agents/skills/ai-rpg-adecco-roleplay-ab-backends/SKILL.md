...
`agentTextPreviewUtf8Base64`). Use the Base64 fields as the source of truth for
evaluation transcripts because Cloud Logging / terminal display can degrade
Japanese text into `????`. Do not trust client-provided Base64 fields; they must
be generated by the server from the sanitized preview text.
