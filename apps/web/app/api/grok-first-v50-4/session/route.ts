...
export async function POST(request: NextRequest) {
  ...
  try {
    const session = await createGrokFirstV50Session({ variant: "v50.4" });
    ...
    logGrokFirstV50ServerEvent({
      ...
      debugTranscriptPreviewEnabled: session.debugTranscriptPreviewEnabled,
    });
    return NextResponse.json(session);
  } catch (error) {
    console.error(
      "grok-first v50.4 session failed",
