...
export function useGrokFirstRoleplayConversation(
  ...
  const emitMetric = useCallback(
    (input: { routePath: GrokFirstV50Metric["routePath"]; guardAction?: GrokFirstV50Metric["guardAction"]; guardReasons?: string[] }) => {
  ...
  const setOutputVolume = useCallback(async (nextVolume: number) => {
