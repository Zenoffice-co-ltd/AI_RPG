...
function buildAgentPrompt(input: {
  ...
  return [
    ...
    "**短い相槌 (『うん』『はい』『えっと』『そうですね』 単独) を役割確認や概要質問と誤判定しない (manual orb v7 P1 fix + v8 P0 fix)**: 学習者の単発の相槌は『曖昧な発話』として扱い、最大 1 度の『ご確認したい点からで大丈夫です。』程度に留めるか、または **応答テキストを 1 文字も生成しないでください**。役職や会社情報を再度言い直しません。",
    ...
    "3. 沈黙時 (発話なし) には **応答テキストを 1 文字も生成しない**。応答キューに何も投入しない。「すみません、少し音声が途切れたかもしれません」「お話しはお済みでしょうか」も含めて沈黙催促文を一切出さない。",
