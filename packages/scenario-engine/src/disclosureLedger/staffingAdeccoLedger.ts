...
export const STAFFING_ADECCO_DISCLOSURE_LEDGER: DisclosureItem[] = [
  {
    ...
    intentDescription:
      "学習者が **明示的に** 役割や立場を確認した時 **だけ**。例：『あなたは誰ですか』『どなたですか』『役職は』『お名前は』『どちら様ですか』。**短い相槌『うん』『はい』『えっと』『そうですね』『なるほど』『あ、』単独 では identity_self を発火させない (manual orb v7 P1 fix)** — そのような曖昧発話には **応答テキストを 1 文字も生成しない (応答キューに何も投入しない)**。役職を 2 回以上同じ会話で言い直さない。**禁止 (manual orb v8 P0): 『（何も返さず…）』『（沈黙）』『（応答なし）』のような括弧付き stage direction / メタ動作描写を発話してはいけない。**",
    allowedAnswer:
  ...
  },
  {
    ...
    intentDescription:
      "学習者が指揮命令者 (課長) の人柄、または合う / 合わない人物像を確認した時。例：『指揮命令者はどんな方ですか』『課長の人柄は』『合わないタイプは』『どんな人が馴染みますか』『NG な性格はありますか』。**指揮命令者の人柄 + 合う/合わないタイプの 1〜2 文だけ** を返す。部署人数・男女比・服装・休憩室は team_atmosphere_question で扱うため、ここでは触れない。",
    allowedAnswer:
  ...
  },
  {
    ...
    intentDescription:
      "学習者が部署構成・部署の雰囲気・男女比・平均年齢・派遣社員数・服装・休憩室など、職場の物理的・組織的環境を確認した時。例：『部署の雰囲気はどうですか』『男女比は』『平均年齢は』『部署は何名ですか』『他に派遣の方はいますか』『服装は』『休憩室はありますか』。**職場環境の事実だけ** を返す。指揮命令者の人柄や合う/合わないタイプは supervisor_personality_question で扱うため、ここでは触れない。",
    allowedAnswer:
  ...
  },
  {
    ...
    intentDescription:
      "学習者 (営業側) が商談上の次アクションを確認した時にだけ発火。例：『次はどう進める？』『今後の進め方は』『次は何を出せばよいですか』。コーチング要求 (`coaching_request`) ではなく、顧客 (人事側) として **自然な次アクションを返す**。受け流したり、質問項目を列挙したりしない。**AI 側 (人事) から自発的に『次はどう進めますか』『どう進めましょうか』と問いかけない (manual orb v7 P1 fix)** — next_step_close は学習者 (営業) が AI に問いかける発話パターンであり、AI 側から発するものではない。学習者が沈黙していても、AI が代わりに進行確認しない。",
    allowedAnswer:
  ...
  },
  {
    ...
    intentDescription: [
      ...
      "**(A)+(B) を満たして closing_summary が発火しても、合意する前に必ず学習者要約に含まれる重要条件をシナリオ真値と照合する。** 真値と異なる項目があれば、まず明確に『違います』と言い、誤っている項目だけを訂正する。",
      "",
    ...
    .map((item) => {
      ...
      const shallowGuards: Record<string, string> = {
        ...
        supervisor_personality_question:
          "**指揮命令者の人柄 (落ち着いて正確性に厳しい) と 合う/合わないタイプ (協調型 OK / 自己流 NG) だけ** を 1〜2 文で答える。部署人数・男女比・服装・休憩室は team_atmosphere_question の責務なので **ここでは出さない**。優先順位・採用条件・競合・決定構造・要約合意文や Adecco / アデコ 強み逆質問を続けて出さない。",
        team_atmosphere_question:
          "**部署構成 (12 名 / 女性 8 / 男性 4 / 30〜40 代) と 派遣スタッフ数 (他に 3 名) と 服装 (オフィスカジュアル) / 休憩室 だけ** を 1〜2 文で答える。指揮命令者の人柄や合う/合わないタイプは supervisor_personality_question の責務なので **ここでは出さない**。優先順位・採用条件・競合・決定構造・要約合意文や Adecco / アデコ 強み逆質問を続けて出さない。",
      };
