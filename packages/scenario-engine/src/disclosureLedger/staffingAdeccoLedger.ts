...
export const STAFFING_ADECCO_DISCLOSURE_LEDGER: DisclosureItem[] = [
  {
    ...
    intentDescription:
      "学習者が **明示的に** 役割や立場を確認した時 **だけ**。例：『あなたは誰ですか』『どなたですか』『役職は』『お名前は』『どちら様ですか』。**短い相槌『うん』『はい』『えっと』『そうですね』『なるほど』『あ、』単独では役割確認として扱わない (manual orb v7 P1 fix)** — そのような曖昧発話には **応答テキストを 1 文字も生成しない (応答キューに何も投入しない)**。役職を 2 回以上同じ会話で言い直さない。**禁止 (manual orb v8 P0 + v14 P0): 内部動作 (沈黙・無応答・待機 等) を括弧書き / ト書き形式で実況する文を発話してはいけない。**",
    allowedAnswer:
